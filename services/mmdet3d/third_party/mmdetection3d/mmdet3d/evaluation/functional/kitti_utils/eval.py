...
_ROTATE_IOU_GPU_AVAILABLE = True


def _rotated_rect_to_cv(box):
    # box: [center_x, center_y, width, height, rotation]
    center = (float(box[0]), float(box[1]))
    size = (max(float(box[2]), 0.0), max(float(box[3]), 0.0))
    angle = float(-box[4] * 180.0 / np.pi)
    return center, size, angle


def _rotated_intersection_area(box1, box2):
    rect1 = _rotated_rect_to_cv(box1)
    rect2 = _rotated_rect_to_cv(box2)
...
def rotate_iou_cpu_eval(boxes, query_boxes, criterion=-1):
    ...
    for i in range(N):
        ...
        for j in range(K):
            ...
            inter = _rotated_intersection_area(boxes[i], query_boxes[j])
            if criterion == -1:
                ...
                iou[i, j] = inter
                continue
            iou[i, j] = 0.0 if denom <= 0 else inter / denom
    return iou


def _rotate_iou_eval(boxes, query_boxes, criterion=-1):
    global _ROTATE_IOU_GPU_AVAILABLE
    if _ROTATE_IOU_GPU_AVAILABLE:
        try:
            from .rotate_iou import rotate_iou_gpu_eval
            return rotate_iou_gpu_eval(boxes, query_boxes, criterion)
        except Exception:
            _ROTATE_IOU_GPU_AVAILABLE = False
...
def bev_box_overlap(boxes, qboxes, criterion=-1):
    riou = _rotate_iou_eval(boxes, qboxes, criterion)
    return riou
...
def d3_box_overlap(boxes, qboxes, criterion=-1):
    rinc = _rotate_iou_eval(boxes[:, [0, 2, 3, 5, 6]],
                            qboxes[:, [0, 2, 3, 5, 6]], 2)
